...
InfidelityRobustnessObjective(;
    H::::Union{EmbeddedOperator, AbstractMatrix{<:Number}, Nothing}=nothing,
    eval_hessian::Bool=false,
    symb::Symbol=:Ũ⃗
)
...
function InfidelityRobustnessObjective(;
    ...
    if H_error isa EmbeddedOperator
        H = unembed(H_error)
        subspace = get_unitary_isomorphism_subspace_indices(H_error)
    else
        H = H_error
...
"""
    PairwiseInfidelityRobustnessObjective(;
        H1::Union{EmbeddedOperator, AbstractMatrix{<:Number}, Nothing}=nothing,
        H2_error::Union{EmbeddedOperator, AbstractMatrix{<:Number}, Nothing}=nothing,
        symb1::Symbol=:Ũ⃗1,
        symb2::Symbol=:Ũ⃗2,
        eval_hessian::Bool=false,
    )
...
function PairwiseInfidelityRobustnessObjective(;
    H1_error::Union{EmbeddedOperator, AbstractMatrix{<:Number}, Nothing}=nothing,
    H2_error::Union{EmbeddedOperator, AbstractMatrix{<:Number}, Nothing}=nothing,
    symb1::Symbol=:Ũ⃗1,
    symb2::Symbol=:Ũ⃗2,
    eval_hessian::Bool=false,
    ...
    if H1_error isa EmbeddedOperator
        H1 = unembed(H1_error)
        subspace1 = get_unitary_isomorphism_subspace_indices(H1_error)
    else
        H1 = H1_error
    ...
    if H2_error isa EmbeddedOperator
        H2 = unembed(H2_error)
        subspace2 = get_unitary_isomorphism_subspace_indices(H2_error)
    else
        H2 = H2_error
